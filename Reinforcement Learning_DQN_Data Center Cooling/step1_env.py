...
class Environment(object):
	...
	def update_env(self, direction, energy_ai, month):
		...
		delta_temperature_noai = 0
		energy_noai = 0
		if (self.temperature_noai < self.optimal_temperature[0]):
			energy_noai = self.optimal_temperature[0] - self.temperature_noai 
			# Note from BREMNER: ADDED TO CODE (Missing From original)
			# delta temperature_noai necessary to properly update the teperature for noai
			delta_temperature_noai = self.optimal_temperature[0] - self.temperature_noai
			...
			## Note from BREMNER: ADDED TO CODE (Missing from original)
			# delta temperature_noai necessary to properly update the teperature for noai
			delta_temperature_noai = self.optimal_temperature[1] - self.temperature_noai
		...
		if (direction == -1): 
			delta_temperature_ai = -1*energy_ai
		elif (direction == 1):
		...
		# Note from BREMNER:
		# self.temperature_noai += delta_intrinsic_temperature <---- OLD
		# ADDED "+delta_temperature_noai" to be consistent with write up. 
		# delta temperature_noai necessary to properly update the teperature for noai

		self.temperature_noai += delta_intrinsic_temperature + delta_temperature_noai


		# GETTING GAME OVER
		# Note from BREMNER: What this function does is during training, if the agent gets
		# to either then min or max range of temperatures it ends that EPOCH.
		# during testing it resets to the closests optimal temperature, which is equivilant to 
		# switching back to the NO_AI policy
